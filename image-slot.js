...
(() => {
  ...
  const stylesheet =
    ...
    '.frame{position:absolute;inset:0;overflow:hidden;background:rgba(0,0,0,.04);container:omslot/size}' +
    ...
    '@container omslot (max-width: 80px) {.empty .cap,.empty .sub{display:none}}' +
    '@container omslot (max-height: 80px) {.empty .sub{display:none}}' +
  ...
  class ImageSlot extends HTMLElement {
    ...
    constructor() {
      ...
      root.innerHTML =
        ...
        '    <div class="cap">Фото требует атрибуции</div></div>' +
    ...
    _render() {
      ...
      this._cap.textContent = this.getAttribute('placeholder') || 'Перетащите картинку';
